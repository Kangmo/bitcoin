...
// kangmo : comment - For getting the median value of the best block height for the 5 peers recently connected.
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    {
    	// kangmo : req - (P2) Check if a peer is sending Version message twice.
        ...
        // kangmo : req - (P2) Check if the version from a peer is not zero.
        if (pfrom->nVersion == 0)
            return false;

        // kangmo : req - (P2) Check if the Version.nonce is generated by us to check if we are connected to ourself.
        ...
        // kangmo : req - (P2) The peer that accepts connections should send Version first.
        // Be shy and don't send version until we hear
        if (pfrom->fInbound)
            pfrom->PushVersion();

        // kangmo : question - what is this?
        ...
        // kangmo : question - why do we set the version at vSend?
        pfrom->vSend.SetVersion(min(pfrom->nVersion, VERSION));
        // kangmo : comment - vRecv.version should be set upon the receival of Verack. Looks like we did not receive Verack until version 209.
        if (pfrom->nVersion < 209)
            pfrom->vRecv.SetVersion(min(pfrom->nVersion, VERSION));

        // kangmo : req - (P1) The peer that connects to another peer should (1) send addr (2) send getaddr if we don't have 1000 addresses yet.
        if (!pfrom->fInbound)
        {
        ...
        // kangmo : req - (P1) Ask blocks for the first connected peer by sending the best block that we have.
        ...
        // For getting the median value of the best block height for the 5 peers recently connected.
        cPeerBlockCounts.input(pfrom->nStartingHeight);
    }


    // kangmo : req - (P1) check if Version message was received before any message.
    ...
    // kangmo : req - (P1) set the minimum version for receiving data upon the receival of a verack message.
    ...
    // kangmo : req - (P1) Keep the received address to the local address db, relay addresses to a limitted number of peers.
    else if (strCommand == "addr")
    {
        ...
        if (pfrom->nVersion < 209)
            return true;
        // kangmo - req : (P1) Ignore addr if we have more than 1000 addressess already and the peer version is less than 31402. (why?)
        if (pfrom->nVersion < 31402 && mapAddresses.size() > 1000)
            return true;
        // kangmo - req : (P2) Check if the received addr message hash more than 1000 addresses.
        if (vAddr.size() > 1000)
        {
        ...
        BOOST_FOREACH(CAddress& addr, vAddr)
        {
            ...
            // kangmo : comment - message(addr) : Add the address to both the in-memory map, and on disk map. Keep the service that the peer (by the address) provides updated.
            AddAddress(addr, 2 * 60 * 60, &addrDB);
            // kangmo : comment - message(addr) : To not send the same address back to the peer, add the address to the list of 'known' addresses.
            pfrom->AddAddressKnown(addr);

            // kangmo : comment - message(addr) : addr.nTime > nSince - relay the address if its time is in the recent 10 minutes.
            // kangmo : comment - message(addr) : !pfrom->fGetAddr - do not relay addresses back to the peer that we requested "getaddr" message upon the receival of Version.
            // kangmo : comment - message(addr) : vAddr.size() <= 10 && addr.IsRoutable() - relay the address if it is routable and we have <= 10 addresses to relay.
            if (addr.nTime > nSince && !pfrom->fGetAddr && vAddr.size() <= 10 && addr.IsRoutable())
            {
                // Relay to a limited number of other nodes
                CRITICAL_BLOCK(cs_vNodes)
                {
                	// kangmo : question - Not sure why we need to send to the same nodes for 24 hours.
        ...
        // kangmo : question - why need to set fGetAddr back to false?
        if (vAddr.size() < 1000)
    ...
    else if (strCommand == "inv")
    {
        vector<CInv> vInv;
        vRecv >> vInv;
        // kangmo : req - message(inv) : The number of inventories should not be greater than 50,000.
        if (vInv.size() > 50000)
        {
            pfrom->Misbehaving(20);
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (!fAlreadyHave)
                pfrom->AskFor(inv); // kangmo : comment - we will send 'getdata' for inventories that we don't have yet.
            else if (inv.type == MSG_BLOCK && mapOrphanBlocks.count(inv.hash)) // kangmo : req - (P1) message(inv) : if the inventory is an orphan block, get the blocks required to connect the orphan to the blockchain.
    ...
    // kangmo : comment - for "getdata" requests, send "block" message for the blocks we have on disk. send "tx" message only if it was put in the relay memory by sendfrom, sendtoaddress, sendmany RPC.
    else if (strCommand == "getdata")
    {
        vector<CInv> vInv;
        vRecv >> vInv;

        // kangmo : req - message(getdata) : The number of inventories should not be greater than 50,000.
        if (vInv.size() > 50000)
        {
            pfrom->Misbehaving(20);
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK) // kangmo : comment - Unpon receival of getdata(blockhash), read the block from disk to send the block message.
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    // kangmo : comment - hashContinue is set by 'getblocks' when it reached at the count limit. Let the peer learn about our best block hash if it requested the hashConitnue block hash.
            ...
            // kangmo : comment - this case is for 'tx' inventory only.
            else if (inv.IsKnownType()) // kangmo : req - (P1) send tx message only if it is in the relay memory. A 'tx' is put into the relay memory by sendfrom, sendtoaddress, sendmany RPC.
            {
    ...
    // kangmo : req - (P1) getblocks should start sending the last block the caller has in our main chain.
    // kangmo : req - (P1) if getblocks stops when it hits the block count limit (500), it should be able to send our best block hash when the peer sends 'getdata' with the stopped hash.
    else if (strCommand == "getblocks")
    {
        ...
        if (pindex)
            pindex = pindex->pnext;
        // kangmo : comment - GetDistanceBack returns the depth(in terms of the sender's blockchain) of the block that is in our main chain. It returns 0 if the tip of sender's branch is in our main chain. We will send up to 500 more blocks from the tip height of the sender's chain.
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            ...
            if (--nLimit <= 0 || nBytes >= SendBufferSize()/2)
            {
                ...
                // kangmo : req - (P1) We reached at the limit. We have to continue later from the block hash we stopped for the peer.
    ...
    // kangmo : req - (P1) getheaders should find the last block the caller has in the main chain to fill the headers message.
    // kangmo : req - (P1) getheaders should send the block header of the stop hash, if the block locator does not have any common block with our main chain.
    else if (strCommand == "getheaders")
    {
        ...
        if (locator.IsNull()) // kangmo : comment - the block locator does not have any common block with our main chain.
        {
...
bool SendMessages(CNode* pto, bool fSendTrickle)
{
    CRITICAL_BLOCK(cs_main)
    {
        ...
        // kangmo : req - (P1) Send ping for every 30 minutes.
        // Keep-alive ping
        ...
        // kangmo : req - (P2) Broadcast my address to all peers every day.
        ...
        // kangmo : req - (P2) if addresses are < 1000, clear addresses that are older than 14 days every 10 minutes.
        ...
        // kangmo : comment - send messages in
        //
        // Message: addr
        //
        if (fSendTrickle)
        ...
        // kangmo : comment - Send only (randomly chosen) 1/4 of all transactions to send.
        //
        // Message: inventory
        //
        vector<CInv> vInv;
        ...
        CRITICAL_BLOCK(pto->cs_inventory)
        {
            ...
            BOOST_FOREACH(const CInv& inv, pto->vInventoryToSend)
            {
                ...
                if (inv.type == MSG_TX && !fSendTrickle)
                {
                    ...
                    // kangmo : question - why don't send our own transactions?
        ...
        // kangmo : comment - Send get data for inventories that we do not have yet.
        //
        // Message: getdata
        //
        ...
        {
        	// kangmo : question - why are we not using the key(timestamp) of the mapAskFor?
